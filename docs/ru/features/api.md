...
Эта функция предоставляет несколько API endpoints для управления кешами. Вы можете получить список элементов кеша, проверить элементы кеша, удалить элементы по ключу или удалить элементы по cache tags.
...
По умолчанию endpoints недоступны без авторизации.

#### На основе токена (x-nuxt-multi-cache-token)

Это авторизация по умолчанию, используемая когда предоставлена опция `api.authorization`. В этом случае токен ожидается в заголовке `x-nuxt-multi-cache-token`:
...
Также возможно [предоставить токен через runtime config](/ru/overview/runtime-config#api-authorization-token):
...
Вы также можете реализовать свою собственную [проверку авторизации через server options](/ru/overview/server-options#custom-api-authorization).

#### Отключена

Вы можете отключить авторизацию, установив значение `api.authorization` в `false`. **Делайте это только если endpoints недоступны публично!**

## Очистка всего

Очищает все из всех кешей.
...
Очистить один или несколько элементов кеша по ключу.

### Пример: Очистка компонента с ключом `Navbar::de--chf`
...
### Пример: Очистка двух конкретных страниц
...
Очистка элементов кеша по cache tags.

::: info

Все теги собираются в течение некоторого времени (по умолчанию: 1 мин), после чего элементы кеша очищаются. Это связано с тем, что cache tags хранятся вместе с элементами. Это означает, что каждый элемент нужно загрузить из кеша и проверить его теги.

Задержка настраивается через опцию `api.cacheTagInvalidationDelay`.

:::

### Пример: Очистка всех элементов кеша с cache tag `language:de`
...
Получить список всех элементов в кеше.

### Пример: Получить список всех кешированных компонентов
...
```json
{
  ...
  "rows": [
    {
      "key": "RandomNumber",
      "data": "<div><h3>Компонент со случайным числом</h3><div>RANDOM_NUMBER__686204309__</div></div>"
    },
    {
      "key": "Navbar:navbar",
      "data": {
...
Проверить элемент кеша. Ключ должен быть предоставлен как query параметр `key`.

### Пример: Получить разметку кешированного компонента с ключом `Navbar::de`
...
```html
<div>
  <h3>Компонент со случайным числом</h3>
...
### Пример: Получить детали кешированного маршрута
